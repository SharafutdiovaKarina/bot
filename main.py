...
def handle_start(message):
    ...
    bot.send_message(message.chat.id, "Приветствую! Используйте /help если у вас возникли вопросы",
                     reply_markup=markup)
...
Я создан в рамках курса по разработке пользовательского интерфейса студенткой группы 4311-22 Шарафутдиновой Карины.
Вот что я могу:
...
/rpi - получить определение рпи
/programmer - получить определение программиста
    """
...
@bot.message_handler(func=lambda message: message.text.lower() == "кафетерий")
def handle_blin(message):
    bot.send_message(message.chat.id,
                     "В кафетерии Б открылся трафик!")


# Обработчик для сообщения "где моя пара"
@bot.message_handler(func=lambda message: message.text.lower() == "локации")
...
def handle_unknown(message):
    bot.send_message(message.chat.id, "Извините, я Вас не понял. Попробуйте еще раз")
